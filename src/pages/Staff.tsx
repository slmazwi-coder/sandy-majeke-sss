...
const staffData: StaffMember[] = [
  { name: 'TBC', position: 'Principal', category: 'Leadership' },
...
const StaffCard = ({ member }: { member: StaffMember }) => (
  <div className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col items-center p-6 text-center border border-[#fee2e2] hover:-translate-y-1">
    <div className="w-24 h-24 rounded-full bg-red-50 border-4 border-[#fca5a5] flex items-center justify-center mb-4 overflow-hidden">
      {member.image ? (
        <img src={member.image} alt={member.name} className="w-full h-full object-cover" />
      ) : (
        <div className="w-full h-full bg-gradient-to-br from-[#A31621] to-[#7A1019] flex items-center justify-center text-white font-bold text-2xl font-serif">
    ...
    {member.subject && (
      <span className="mt-2 inline-block bg-red-50 text-school-blue text-xs font-medium px-3 py-1 rounded-full">
        {member.subject}
      </span>
    )}
  </div>
...
export const Staff = () => {
  ...
  return (
    <div className="min-h-screen py-12 px-4" style={{ background: '#fef2f2' }}>
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-extrabold tracking-tight mb-3 text-school-blue">Our Staff</h1>
          <p className="text-gray-500 max-w-xl mx-auto">Meet the dedicated educators and support staff who make Sandy Majeke SSS thrive every day.</p>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-10">
          {categories.map(cat => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-bold border-2 transition-all ${activeCategory === cat ? 'bg-school-blue text-white border-school-blue' : 'bg-white text-school-blue border-school-blue hover:bg-red-50'}`}
            >
              {cat}
            </button>
          ))}
