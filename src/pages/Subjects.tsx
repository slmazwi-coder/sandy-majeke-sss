    ...
    <div className="py-12 sm:py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p className="text-center text-gray-500 max-w-2xl mx-auto mb-10 -mt-4">
          We offer a comprehensive CAPS curriculum from Grades 8 – 12, covering all core and elective subjects required for the National Senior Certificate.
        </p>
        ...
        <div className="flex flex-wrap gap-3 justify-center mb-10">
          {subjects.map(s => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all border-2 ${activeCategory === s.category ? 'bg-school-blue text-white border-school-blue' : 'bg-white text-school-blue border-school-blue hover:bg-red-50'}`}
            >
              {s.category}
            </button>
          ))}
        ...
        {/* Pass rate */}
        <div className="mt-12 bg-gradient-to-r from-school-blue to-[#7A1019] rounded-3xl p-8 text-center text-white">
          <p className="text-white/70 uppercase tracking-widest text-sm font-bold mb-2">NSC Matric Pass Rate</p>
          <p className="text-5xl font-bold mb-1 text-white">96.1%</p>
          <p className="text-white/70">Class of 2024</p>
        </div>
      </div>
    </div>
  );
